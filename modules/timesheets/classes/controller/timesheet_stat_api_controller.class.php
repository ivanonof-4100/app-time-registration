...
use App\Modules\Timesheets\Classes\Model\TimesheetStat;
use Common\Classes\Datetime\CustomDateTime;

/**
 * Filename     : timesheet_stat_api_controller.class.php
 * Language     : PHP v7.4+
 * Date created : Ivan, 25/01-2025
 * Last modified: Ivan, 25/01-2025
 ...
 */
class TimesheetStatApiController extends ApiController {
     ...
     * @return TimesheetStatApiController
     */
    public static function getInstance(string $p_lang ='da', string $p_charset ='utf8', StdApp $p_appInstance) : TimesheetStatApiController {
        return new TimesheetStatApiController($p_lang, $p_charset, $p_appInstance);
    }

    /**
     * CRUD Read-operation. Get stats for the given employee and within the given time-period.
     * @return void
     */
    public function handleRequest_readList() : void {
        $url = Url::getInstance();
        // Make sure that only GET-method
        if (!$url->isRequestMethod_GET()) {
          ...
          if ($arrInputParam_employeeUUID['is_set'] && $arrInputParam_employeeUUID['is_valid']) {
            $timesheetStat = TimesheetStat::getInstance();
            $arrRows = $timesheetStat->retriveTimesheetStats_onlyRegistered($dbAbstractionInstance, $arrInputParam_employeeUUID['value'], $yearFromDate->getFormatedISODate(), $yearToDate->getFormatedISODate());
            ...
            if (!$arrInputParam_employeeUUID['is_set']) {
              $this->sendHttpResponse(ResponseCode::HTTP_BAD_REQUEST, 'Mandatory parameter employee_uuid was missing ...');
            } else {
              $this->sendHttpResponse(ResponseCode::HTTP_BAD_REQUEST, 'Mandatory parameter employee_uuid was not valid ...');
            }
          }
        }
    }
